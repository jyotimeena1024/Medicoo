// ths file os used to display total earning , total number of patients , latest appointments 
                ...
                <div className='pt-4 border border-t-0'>
                  {
                    dashData.latestAppointments.map((item,index)=>(
                      <div className='flex items-center px-3 py-3 gap-3 hover:bg-gray-100' key={index} >
                        <img className='rounded-full w-10' src={item.userData.image} alt="" />
                        <div className='flex-1 text-sm'>
                          <p className='text-gray-800 font-medium' >{item.userData.name}</p>
                          <p className='text-gray-600 font-medium' >{slotDateFormat(item.slotDate)}</p>
                        </div>
                        {
                            item.cancelled
